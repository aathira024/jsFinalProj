...
function getCartItems() {
  ...
  this.cartItems = this.productList.filter((v) => {
    const product = cartItems.find((b) => b.productId == v.id);
    return product != null;
  });

  let totalAmount = 0;
  const itemcontainer = document.getElementById("checkoutcardlist");
  if (this.cartItems.length > 0) {
    this.cartItems.forEach((element) => {
      const localCartItem = cartItems.find((b) => b.productId == element.id);
      const itemBlock = document.createElement("div");
      itemBlock.classList.add("chekout-img-container");

      const image = document.createElement("img");
      image.setAttribute("src", element.preview);

      const descriptionSection = document.createElement("div");
      descriptionSection.classList.add("item-description-section");
      const title = document.createElement("h4");
      title.classList.add("margin-none");
      title.innerHTML = element.name;
      descriptionSection.appendChild(title);

      const quantity = document.createElement("h3");
      quantity.classList.add("margin-none", "font-weignt-100");
      quantity.innerHTML = `x${localCartItem.quantity}`;

      const amount = document.createElement("h3");
      amount.classList.add("margin-none", "font-weignt-100");

      amount.innerHTML = `Amount : ${element.price}`;
      descriptionSection.appendChild(quantity);
      descriptionSection.appendChild(amount);
      itemBlock.appendChild(image);
      itemBlock.appendChild(descriptionSection);

      itemcontainer.appendChild(itemBlock);
      totalAmount += element.price * localCartItem.quantity;
    });
  } else {
    const itemBlock = document.createElement("div");
    itemBlock.classList.add("chekout-no-item");
    itemBlock.innerHTML = "No item in the cart !";
    itemcontainer.appendChild(itemBlock);
  }
...
setInterval(() => {
  ...
  if (cartItems.length == 0) {
    ...
    btn.querySelector("a").classList.add("disabled");
  }
}, 100);
getCartItems();
