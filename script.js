...
function allcardcreate(item) {
  const link = document.createElement("a");
  link.setAttribute("href", `./productinfo2.html?id=${item.id}`);
  link.classList.add("cardinfo");
...
function createlistofitems(array, id) {
  const itemcards = document.querySelector(`#${id} > .card`);
  for (let i = 0; i < array.length; i++) {
    const cardinfo = allcardcreate(array[i]);
    itemcards.appendChild(cardinfo);
  }
}
function firstfunction() {
  const cloths = productList.filter((item) => !item.isAccessory);
  const accessoris = productList.filter((item) => item.isAccessory);
  createlistofitems(cloths, "clotingcards");
  createlistofitems(accessoris, "accessoriescards");
}

function getProduct() {
  const params = new URLSearchParams(location.search);
  const productid = params.get("id");
  const selectedProduct = productList.find((i) => i.id === productid);
  console.log("selectedProduct", selectedProduct);
  const subImageGalerry = document.querySelector(".previewgallery");
  for (let i = 0; i < selectedProduct.photos.length; i++) {
    const image = document.createElement("img");
    image.setAttribute("id", "image" + i);
    image.setAttribute("src", selectedProduct.photos[i]);
    image.onclick = function () {
      onSubImageClick(selectedProduct.photos, i);
    };
    subImageGalerry.appendChild(image);
  }
  const proocontain = document.getElementById("product-itemcontainer");
  const notfoundcotainer = document.getElementById("product not found");
  const addcatrbtn = document.getElementById("addtocarrt");

  if (selectedProduct) {
    const { name, photos, price, preview, brand, description } =
      selectedProduct;

    proocontain.classList.replace("hide", "show");
    notfoundcotainer.classList.replace("show", "hide");
    const images = proocontain.querySelector(".product-privewimages");
    const protitle = proocontain.querySelector(".pro-title");
    const probrand = proocontain.querySelector(".pro-brand");
    const proprice = proocontain.querySelector(".pro-price");
    const prodescr = proocontain.querySelector(".pro-descri");
    const dectext = proocontain.querySelector(".decretext");

    const mainImg = photos.length === 0 ? photos[0] : preview;
    images.setAttribute("src", mainImg);

    protitle.innerHTML = name;
    probrand.innerHTML = brand;
    proprice.innerHTML = price;
    prodescr.innerHTML = description;
  } else {
    proocontain.classList.replace("show", "hide");
    notfoundcotainer.classList.replace("hide", "show");
    addcatrbtn.classList.replace("show", "hide");
  }
}
...
(function () {
  ...
  if (pathname.includes("index.html")) {
    firstfunction();
  } else if (pathname.includes("productinfo2.html")) {
    getProduct();
  }
})();

function onSubImageClick(allPhotos, selectedPhotoIndex) {
  const mainImageElement = document.querySelector(".product-privewimages");
  mainImageElement.setAttribute("src", allPhotos[selectedPhotoIndex]);
  //setting Border
  const allSubGalImages = document
    .querySelector(".previewgallery")
    .querySelectorAll("img");
  for (let k = 0; k < allSubGalImages.length; k++) {
    allSubGalImages[k].classList.remove("highLight");
    if (k == selectedPhotoIndex) {
      allSubGalImages[k].classList.add("highLight");
    }
  }
}
...
function addCartItem() {
  ...
  if (cartItems.length > 0) {
    const existingItem = cartItems.find((v) => v.productId == productid);
    if (existingItem != null) {
      existingItem.quantity++;
      console.log(" this.cartItems add qualtitu", cartItems);
  ...
  console.log(" this.cartItems new item", cartItems);
  localStorage.setItem("cartItem", JSON.stringify(cartItems));
}
